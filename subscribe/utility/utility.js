...
document.getElementById("clientIdInput").value = "SD-" + makeid();
...
function onConnected(reconnect, uri) {
  ...
  logMessage(
    "INFO",
    "Client Has now connected: [Reconnected: ",
    reconnect,
    ", URI: ",
    uri,
    "]"
...
function onMessageArrived(message) {
  ...
  var table = document
    .getElementById("incomingMessageTable")
  ...
  if (!document.getElementById(message.destinationName)) {
    var lastMessageTable = document
      .getElementById("lastMessageTable")
      .getElementsByTagName("tbody")[0];
    ...
    newlastMessageRow.insertCell(1).innerHTML = safeTagsRegex(
      message.payloadString
    );
...
function connect() {
  ...
  var automaticReconnect = document.getElementById("automaticReconnectInput")
    .checked;
...
function unsubscribeSuccess(context) {
  logMessage(
    "INFO",
    "Unsubscribed. [Topic: ",
    context.invocationContext.topic,
    "]"
  );
}
